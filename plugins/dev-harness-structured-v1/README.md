...
| 영역 | 고정 내용 |
|------|-----------|
| **디렉토리·파일** | `docs/PRD.md`·`FeatureSpec.md`·`UserStories.md`·`roadmap.md`·`traceability.md`·`api-contract.md`, `docs/architecture/{architecture,adr}.md`, `docs/reviews/*`, `docs/pm/*` 로 **고정** |
| **검증 보고 명명** | 설계/문서: `{planning\|roadmap\|architecture}_review_{n}.md`, 설계 보안: `architecture_security_review_{n}.md`, 구현(스프린트): `{code_review\|qa\|security_review}_s{N}_{n}.md` |
| **고정 규칙** | ① **`docs/pm/progress.html`만 HTML**(통합 현황판·단일 진입점), 나머지 산출물은 전부 마크다운  ② **ADR은 `adr.md` 한 문서에 `ADR-001`… 누적**(결정마다 파일 분리 금지) |
| **섹션 구조** | 각 산출물의 표준 섹션 순서·표 컬럼·Mermaid/텍스트 배지 사용을 에이전트 정의에 명시 |

> **산출물 형식**: 모든 문서는 마크다운(`.md`)이고 구조·흐름은 Mermaid로 그린다. **유일한 예외는 `docs/pm/progress.html`**.
> 열람·공유용 HTML이 필요하면 `artifacts-to-html` 스킬이 요청 시 마크다운을 자족형 HTML로 내보낸다.
>
...
- **스킬 3종** (`skills/`):
  ...
  - `/dev-harness-structured-v1:artifacts-to-html` — 요청 시 마크다운 산출물을 현재 방식의 자족형 HTML로 빠르게 내보내기

## 산출물 구조 (고정 — 모든 프로젝트 동일) ★

```
docs/
├── PRD.md                    # 제품 요구사항 문서          ← product-planner
├── FeatureSpec.md            # 기능 명세서 (F-1, F-2…)     ← product-planner
├── UserStories.md            # 사용자 스토리 백로그(에픽별)  ← product-planner
├── roadmap.md                # 개발 로드맵 (S0..Sn)        ← tech-lead (구현)
├── api-contract.md           # 확정 API 계약 (FE↔BE 단일 출처) ← tech-lead 중재 / backend-dev (구현)
├── traceability.md           # 추적성 매트릭스             ← PM
├── architecture/
│   ├── architecture.md       # 시스템 아키텍처 설계서       ← chief-architect
│   └── adr.md                # ADR 모음(ADR-001… 누적)     ← chief-architect
├── reviews/                  # 모든 검증 보고
│   ├── planning_review_{n}.md
│   ├── roadmap_review_{n}.md
│   ├── architecture_review_{n}.md
│   ├── architecture_security_review_{n}.md
│   ├── code_review_s{N}_{n}.md
│   ├── qa_s{N}_{n}.md
│   └── security_review_s{N}_{n}.md
└── pm/                       # PM 산출물
    ├── progress.html         # 통합 진행 현황판 — 단일 진입점·유일한 HTML(항상 생성·유지) ★
    ├── codebase-audit.md     # brownfield 감사 (해당 상황에만)
    ├── change-log.md         # 변경 이력(기존 산출물 변경 시)
    └── meeting-{주제}.md      # 회의록
```
...
**유지 규칙(슬림)** — 현황판은 **파생물**이라 원천(`reviews/`·`traceability.md`·`roadmap.md`)을 복제하지 않고
...
## 산출물 형식 — Markdown 표준 (Mermaid 시각화)

**모든 문서 산출물은 GitHub-flavored Markdown(`.md`)** 으로 생성된다(소스 코드 제외).
**유일한 예외는 `docs/pm/progress.html`** — 통합 진행 현황판만 자족형 HTML로 유지한다(시각 대시보드·단일 진입점).
평상시 산출은 마크다운으로 가볍게 두고, diff·검색·버전관리 친화적으로 관리한다.

**형식 규칙**
1. **사용자 템플릿 우선** — 프로젝트의 **`docs/Templates/`** 에 해당 유형 마크다운 템플릿(`*.md`)이 있으면 그 구조·섹션 구성을 따른다.
   - 예: `docs/Templates/prd.md`, `docs/Templates/architecture.md`, 공통 베이스 `docs/Templates/_base.md`
2. **표준 마크다운 규칙** — 외부 의존 없는 순수 마크다운. `# {프로젝트명} — {문서 유형}` 헤더 + 한 줄 요약 + 메타 줄, 길면 상단에 목차(앵커).

**시각 요소 규칙** — 구조·흐름·타임라인·신뢰경계는 **Mermaid 코드블록**(`flowchart`·`sequenceDiagram`·`gantt`·`stateDiagram`),
항목 나열은 **마크다운 표**, 판정/등급/우선순위는 **텍스트 배지**(`**PASS**`/`**FAIL**`·`P1`/`P2`/`P3`·`[치명]`/`[높음]`/`[중간]`/`[낮음]`)로 표현한다.

### 요청 시 HTML 내보내기 (`artifacts-to-html` 스킬)

현재 방식의 자족형 HTML(다크 테마·인라인 CSS·SVG·카드/배지)로 열람·공유하고 싶으면
**`/dev-harness-structured-v1:artifacts-to-html`** 스킬이 마크다운 산출물을 **같은 경로의 `.html`**로 빠르게 렌더한다
(`docs/PRD.md` → `docs/PRD.html`, 표→`<table>`·배지→색상 배지·Mermaid→인라인 SVG). 마크다운이 원천이고 HTML은 스냅샷이다.

**HTML 스타일의 단일 출처**는 동봉 [`templates/artifact-base.html`](templates/artifact-base.html)의 `:root` CSS 변수
(다크 테마·시스템 폰트 `"Segoe UI","Malgun Gothic"`·헤더+카드/표·**인라인 CSS**, 외부 의존 없음)다 — 이 스킬이 그 팔레트를 따른다.
이 파일을 `docs/Templates/_base.html`로 복사해 커스터마이즈하면 내보내기가 그 템플릿을 따른다.

**통합 진행 현황판**은 처음부터 HTML이라 변환 대상이 아니다. 전용 스켈레톤이 동봉돼 있다:
[`templates/progress-dashboard.html`](templates/progress-dashboard.html) (→ `docs/pm/progress.html`로 산출.
커스터마이즈하려면 `docs/Templates/progress.html`로 복사).
